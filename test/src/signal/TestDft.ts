...
function checkDftRealResult (a: Float64Array, b: ComplexArray, eps: number) {
...
function checkDftResult (a: ComplexArray, b: ComplexArray, eps: number) {
   const n = a.length;
...
// Use random numbers to test dftRealSpectrum() and goertzelSpectrum() with iDftRealSpectrum().
...
function checkDftSpectrum (a: Float64Array) {
   const b1 = Dft.dftRealSpectrum(a, true);
   const b2 = Goertzel.goertzelSpectrum(a, true);
   verifyEqualComplex(b1, b2, 1E-10);
   const c = Dft.iDftRealSpectrum(b1, a.length);
